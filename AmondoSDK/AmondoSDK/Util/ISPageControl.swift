...
private extension ISPageControl {
    ...
    func updateDotLayersLayout() {
        ...
        let y = (bounds.size.height - diameter) * (numberOfPages <= limit  ? 1.0 : 0.5)
    ...
    func setupDotLayersPosition() {
        ...
        dotLayers.enumerated().filter{ $0.offset != centerIndex }.forEach {
            ...
            print( $0.element.position.y)
            $0.element.position = CGPoint(x: centerLayer.position.x + interval * CGFloat(index), y: 25)
        }
    }
